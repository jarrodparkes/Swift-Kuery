...
public class Select: Query {
    ...
    public static func distinct(_ fields: Field..., from table: Table) -> Select {
        let selectQuery = Select(fields, from: table)
    ...
    /// - Returns: A new instance of Select with `distinct` flag set.
    public static func distinct(_ fields: [Field], from table: Table...) -> Select {
        let selectQuery = Select(fields: fields, from: table)
        selectQuery.distinct = true
        return selectQuery
    }
    ...
    public static func distinct(_ fields: Field..., from tables: [Table]) -> Select {
        let selectQuery = Select(fields: fields, from: tables)
        selectQuery.distinct = true
        return selectQuery
    }
    ...
    public func having(_ clause: QueryHavingProtocol) -> Select {
        cachedQuery = nil
        if havingClause != nil {
            syntaxError += "Multiple having clauses. "
        } else {
            havingClause = clause
        }
        return self
    ...
    public func order(by clause: [OrderBy]) -> Select {
        cachedQuery = nil
        if orderBy != nil {
            syntaxError += "Multiple order by clauses. "
        }
        else {
            orderBy = clause
        }
    ...
    public func group(by clause: [Column]) -> Select {
        cachedQuery = nil
        if groupBy != nil {
            syntaxError += "Multiple group by clauses. "
        }
        else {
            groupBy = clause
        }
    ...
    public func limit(to newLimit: Int) -> Select {
        cachedQuery = nil
        if rowsToReturn != nil {
            syntaxError += "Multiple limits. "
        }
        else {
            rowsToReturn = newLimit
        }
        return self
    }
    ...
    public func on(_ conditions: QueryFilterProtocol) -> Select {
        cachedQuery = nil
        guard joins.count > 0 else {
            syntaxError += "On clause set for statement that is not join. "
            return self
        }
        
        if joins.last?.on != nil {
            syntaxError += "Multiple on clauses for a single join."
        } else if joins.last?.using != nil {
            syntaxError += "An on clause is not allowed with a using clause for a single join."
        }
        else {
            joins[joins.count - 1].on = conditions
        }
        return self
    }
    ...
    public func using(_ columns: [Column]) -> Select {
        cachedQuery = nil
        guard joins.count > 0 else {
            syntaxError += "Using clause set for statement that is not join. "
            return self
        }
        
        if joins.last?.using != nil {
            syntaxError += "Multiple using clauses for a single join."
        } else if joins.last?.on != nil {
            syntaxError += "A using clause is not allowed with an on clause for single join."
        }
        else {
            joins[joins.count - 1].using = columns
        }
        return self
    }
    ...
    public func join(_ table: Table) -> Select {
        cachedQuery = nil
        joins.append((.join(table), nil, nil))
        return self
    }
    ...
    public func leftJoin(_ table: Table) -> Select {
        cachedQuery = nil
        joins.append((.left(table), nil, nil))
        return self
    }
    ...
    public func crossJoin(_ table: Table) -> Select {
        cachedQuery = nil
        joins.append((.cross(table), nil, nil))
        return self
    }
    ...
    public func naturalJoin(_ table: Table) -> Select {
        cachedQuery = nil
        joins.append((.natural(table), nil, nil))
        return self
    }
    ...
    public func rawJoin(_ raw: String, _ table: Table) -> Select {
        cachedQuery = nil
        joins.append((.raw(raw, table), nil, nil))
        return self
    }
